...
# --- Adapté (copié depuis generer_pptx_S00.py, même adaptation, 14/09) --------------
# La consigne enseignant impose le nom de source
# Seances/S03_Arborescence-et-perspective_Slides.md (pas le S03_Slides.md que la
# dérivation par défaut du STEM aurait produit) -- source et sortie fixées
# explicitement plutôt que dérivées, seule adaptation nécessaire ici.
STEM = "S03"
SLIDES_MD = os.path.join(HERE, "S03_Arborescence-et-perspective_Slides.md")
...
def _parse_figures(text):
    ...
    Adapté le 14/09 (LOT C) : le modèle n'acceptait qu'un chemin commençant
    littéralement par `Images/` (sibling de Seances/, vrai pour un dépôt de cours
    autonome). TD-Helice-Marine range sa galerie sous `Helice/Images/galerie/`, pas
    `Images/` à la racine -- regex élargie pour accepter aussi ce préfixe, sans
    changer la résolution (toujours relative à la racine du dépôt, HERE/..).
    """
    paths = []

...
| Diamètre D | **≈ 0,227 m** (R_tip = 0,1137 m) | Rayon max $\sqrt{x^2+z^2}$ sur l'ensemble des points de `propellerTip` (27 880 points). Percentiles de contrôle : p99 = 0,1120 m, p99,9 = 0,1135 m, p100 = 0,1137 m — progression lisse, pas un point isolé aberrant (720 points au-delà de r=0,11 m). | ~1 % (résolution du maillage de surface à la pointe de pale). **Écart avec la documentation existante (D = 0,2 m, `MATERIAU-INTRO_TD-Helice.md` §2) : ~14 %, non expliqué, signalé ici sans être corrigé ailleurs — arbitrage enseignant nécessaire avant de rebâtir la courbe J dessus.** Un premier calcul rapide sur la boîte englobante (bounding box) des patchs propeller avait donné ≈0,190 m — plus proche de la valeur documentée mais **faux** : la pointe de pale au rayon maximal n'est pas alignée avec les axes X ou Z, une boîte englobante la sous-estime. Piège méthodologique à ne pas reproduire. |
